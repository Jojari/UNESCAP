...
###################################### Directory ######################################
directory = "E:\\2020 Lab\\UNESCAP_3\\ArcGIS\\OD_MATRIX\\Bing\\"       #TODO. to revise
...
with open(directory + filename) as f:
    ...
    for i, row in enumerate(k):
        if i > 0:
            ...
            for info in itineraryItems:
                if info is not None:
                    n += 1
                    stage = str(num) + "_" + str(n)

                    try:
                        speed = info["travelDistance"] / info["travelDuration"]
                    except ZeroDivisionError:
                        speed = 0

                    print(num, stage, stage_count, O, D, O_x, O_y, D_x, D_y, item["travelDistance"],
                          item["travelDuration"], info["instruction"]["text"],
                          info["maneuverPoint"]["coordinates"][1], info["maneuverPoint"]["coordinates"][0],
                          info["travelDistance"], "km", info["travelDuration"], "sec")
                    INFO = [num, stage, stage_count, O, D, O_x, O_y, D_x, D_y, item["travelDistance"],
                            item["travelDuration"], info["instruction"]["text"],
                            info["maneuverPoint"]["coordinates"][1], info["maneuverPoint"]["coordinates"][0],
                            info["travelDistance"], info["travelDuration"]]
                    kk.writerow(INFO)

                    # print(item["instruction"]["text"], item["travelDistance"], item["travelDuration"])
                    if item is None:
                        n += 1
                        print(n, "###############FAIL###############")
